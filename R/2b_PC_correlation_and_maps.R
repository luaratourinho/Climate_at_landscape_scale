...
###### PC tradicional
planilha_correlacao <- read.table(paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/results_all_overall_indices.txt"))
...
plan_correl_order <- planilha_correlacao[order(as.integer(planilha_correlacao$numeros),decreasing = FALSE), ]

#salvar nova planilha de resultado
write.table(plan_correl_order, paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/results_all_overall_indices_corrigido.txt"))


#abre planilha dos resultados
presente_Ocapu <- read.table(paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/results_all_overall_indices_corrigido.txt"))
...
write.table(presente_Ocapu, paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/results_all_overall_indices_norm.txt"))


# Rodar script 3_Calculating_mean_suitability_vs_hexagons ----------------------


####################adequabilidades
adeq_presente <- read.table(paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/Adeq_media_por_hexagono_current.txt"))
adeq_futuro <- read.table(paste0("~/mestrado/Nodes_dists_PC/", sp.names[7], "/Adeq_media_por_hexagono_future.txt"))


################## PC x adequabilidades
PC_x_adeq_presente <- unlist(presente_Ocapu$PC_norm * adeq_presente)
PC_x_adeq_futuro <- unlist(presente_Ocapu$PC_norm * adeq_futuro)
...
plot(PC_x_adeq_presente, PC_x_adeq_futuro, ylab="Future PCxClima", xlab="Current PCxClima", las=1, bty="l", tcl=0, cex=1, ylim=c(0,1), xlim=c(0,1), family="serif")

correlacao <- cor.test(PC_x_adeq_presente, PC_x_adeq_futuro)
coef.de.correlacao <- correlacao$estimate #ver as estimativas da correlacao
text(paste("r =", round(coef.de.correlacao,3)), x=0.8, y=0.2, cex=1.5) #colocar o "r" no grafico
abline(lm(PC_x_adeq_futuro~PC_x_adeq_presente)) #colocar a linha no grafico
regressao <- lm(PC_x_adeq_futuro~PC_x_adeq_presente) # calculo inclinacao da reta
inclinacao <- as.numeric(regressao$coefficients[2])
text(paste("b =", round(inclinacao,3)), x=0.8, y=0.3, cex=1.5) 
