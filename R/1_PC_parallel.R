...
# Calculating just nodes (now distances is ready)
...
for(sp.n in sp.names[1:71]){
  ...
  system.time( #Monitorando o tempo de processamento do loop
    foreach(i = 1:length(shapelist), .packages = c("rgdal", "raster", "sp", 
                                                   "rgeos", "reshape")) %dopar% {
                                                     ...
                                                     # The distance is the same for all species, calculate only once
  ...
  dir.create(paste0("/home/users/luaratourinho/mestrado/Nodes_dists_PC/", sp.n))
  current_folder <- "."
  target_dir <- paste0("/home/users/luaratourinho/mestrado/Nodes_dists_PC/", sp.n)
...
file <- list.files(current_folder, ".txt")
for (i in 1:length(file)) {
  file2 <- list.files(current_folder, ".txt")
  file.remove(file2)
}
